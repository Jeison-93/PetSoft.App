...
import { SESSION_LS_NAME, STORAGE_LS_ADMON, STORAGE_LS_USER } from '../../models/generic/conts';
...
export class ServicesBoardComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private clientAppointmentServices: ClientAppoitmentServices, private clientServices: ClientServices,
    private localStorageServices : LocalStorageServices, private genericListServices : GenericListService,
    private snackBar: MatSnackBar, private petsServices: PetServices, 
  ...
  private getData(){
    if(this.form.valid){
      this.clientAppointmentServices.GetAppointments(this.form.value).subscribe((res)=>{
          if(res.isSuccessful)
          {
  ...
  addAppointment(){
    ...
    editAppointment(row : any){
      const encryptedValue = this.encryptionService.encryptToken(JSON.stringify(row));
