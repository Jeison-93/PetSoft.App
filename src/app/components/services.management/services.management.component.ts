...
import { SESSION_LS_NAME, STATE_SERVICE_ATEN, STATE_SERVICE_CONF } from '../../models/generic/conts';
...
constructor(private clientAppointmentServices: ClientAppoitmentServices, private localStorageServices : LocalStorageServices,
  private snackBar: MatSnackBar,private encryptionService: EncryptionService
){
  ...
  private getData(){
      ...
      this.clientAppointmentServices.GetAppointmentsByState(STATE_SERVICE_CONF).subscribe((res)=>{
          if(res.isSuccessful)
          {
  ...
  changeState(row : any){
    this.clientAppointmentServices.ChangeState(row.id, STATE_SERVICE_ATEN, this.user).subscribe((res)=>{
      if(res.isSuccessful)
      {
