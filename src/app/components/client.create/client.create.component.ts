...
import { MapperRequestClientDTO, MapperRequestUpdateClientDTO } from '../../utils/mapper.functions';
...
form: FormGroup;
  ...
  constructor(private fb: FormBuilder,private router: Router,  private genericListServices: GenericListService, 
  ...
  private getGenericList(){
    this.genericListServices.GetGenericTable('DocumentType').subscribe((res)=>{
      if(res.isSuccessful)
      {
...
OnSave(){
  this.clientServices.Save(MapperRequestClientDTO(this.form.value)).subscribe(res => {
...
OnUpdate(){
  if (this.form.valid) {
    this.clientServices.Update(MapperRequestUpdateClientDTO(this.form.value, this.data.user.id)).subscribe(res => {
      if(res.isError || !res.isSuccessful){
        this.responseMessage =  res.errorMessage != null ? res.errorMessage : res.message;
        this.showModal= '30%'; 
        this.showSnackbar();
