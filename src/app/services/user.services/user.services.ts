...
export class UserServices {
  ...
  GeUserById(id: number) {
    return this.http
      .get<any>(
        `${this.configService.configValue.urlApiUser}GetUserById?Id=${id}`
      )
      .pipe(
        retry(0),
        catchError(this.handleError),
  ...
  Save(params: any): Observable<any> {
    return this.http
      .post<any>(`${this.configService.configValue.urlApiUser}Save`, params)
      .pipe(
        retry(0),
        catchError(this.handleError),
        map((response) => {
  ...
  Update(params: any): Observable<any> {
    debugger;
    return this.http
      .put<any>(`${this.configService.configValue.urlApiUser}Update`, params)
      .pipe(
        retry(0),
        catchError(this.handleError),
        map((response) => {
  ...
  ChangeState(params: any): Observable<any> {
    return this.http
      .put<any>(
        `${this.configService.configValue.urlApiUser}ChangeState?Id=${params}`,
        params
      )
      .pipe(
        retry(0),
