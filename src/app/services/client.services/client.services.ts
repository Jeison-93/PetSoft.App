...
export class ClientServices {
  ...
  GetAllClients() {
    return this.http
      .get<any>(`${this.configService.configValue.urlApiClient}GetAll`)
      .pipe(
        retry(0),
  ...
  GetActiveClients() {
    return this.http
      .get<any>(`${this.configService.configValue.urlApiClient}GetState`)
      .pipe(
        retry(0),
        catchError(this.handleError),
        map((response) => {
  ...
  Save(params: any): Observable<any> {
    return this.http
      .post<any>(`${this.configService.configValue.urlApiClient}Save`, params)
      .pipe(
        retry(0),
        catchError(this.handleError),
        map((response) => {
  ...
  Update(params: any): Observable<any> {
    return this.http
      .put<any>(`${this.configService.configValue.urlApiClient}Update`, params)
      .pipe(
        retry(0),
        catchError(this.handleError),
        map((response) => {
  ...
  ChangeState(params: any): Observable<any> {
    return this.http
      .put<any>(
        `${this.configService.configValue.urlApiClient}ChangeState?Id=${params}`,
        params
      )
      .pipe(
        retry(0),
